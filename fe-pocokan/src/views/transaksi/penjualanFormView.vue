...
const selectCustomer = (cus: any) => {
  if (!cus) return;
  form.value.cusKode = kode || cus.Kode || cus.cusKode || "";
  form.value.cusNama = nama || cus.Nama || cus.cusNama || "";
  form.value.cusAlamat = alamat || cus.Alamat || "";
  form.value.cusTelp = telp || cus.Telp || "";
  form.value.cusTop = Number(top ?? cus.Top ?? cus.cusTop ?? 0);
