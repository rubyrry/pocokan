...
import { IconBuildingStore, IconDownload } from "@tabler/icons-vue";
import BaseBrowse from "@/components/BaseBrowse.vue";
import { useBrowse } from "@/composables/useBrowse";
import { cabangApi, type Cabang } from "@/api/master/cabangApi";
import { exportToExcel } from "@/utils/exportExcel";

const MENU_ID = "95"; // Sesuaikan ID menu Cabang
...
const {
  ...
  canExport,
  fetchData,
} = useBrowse<Cabang>({ menuId: MENU_ID, fetchApi: cabangApi.getAll });

const headers = [
  { title: "Kode", key: "kode", width: "100px", align: "center" },
  { title: "Nama Cabang", key: "nama", minWidth: "200px" },
...
const emptyForm = () => ({ isEdit: false, kode: "", nama: "", alamat: "", kota: "" });
...
const validateForm = (): boolean => {
  ...
  if (!form.value.kode.trim()) e.kode = "Kode cabang wajib diisi.";
  if (!form.value.nama.trim()) e.nama = "Nama cabang wajib diisi.";
  errors.value = e;
...
const openCreate = () => {
  ...
  errors.value = {};
  dialogTitle.value = "Tambah Cabang";
  dialog.value = true;
};
...
const handleSave = async () => {
  ...
  isSaving.value = true;
  try {
    await cabangApi.save(form.value);
...
const handleDelete = async (item: Cabang) => {
  try {
    await cabangApi.delete(item.kode);
  ...
  <v-dialog v-model="dialog" max-width="600" persistent>
    <v-card rounded="lg">
      <v-card-title class="d-flex align-center gap-2 pa-3" style="background:#3B5998; color:white;">
        <IconBuildingStore :size="20" /> {{ dialogTitle }}
      </v-card-title>
      ...
      <v-card-text class="pa-4 pt-4">
        <div class="grid-fields-container">
          ...
          <div class="f-row">
            <label class="f-lbl">Kode Cabang <span class="req">*</span></label>
            <input
              type="text"
              v-model="form.kode"
              class="f-inp-native"
          ...
          <div class="f-row">
            <label class="f-lbl">Nama Cabang <span class="req">*</span></label>
            <input
              type="text"
              v-model="form.nama"
              class="f-inp-native"
